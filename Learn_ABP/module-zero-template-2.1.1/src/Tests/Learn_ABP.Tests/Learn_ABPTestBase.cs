...
namespace Learn_ABP.Tests
{
    public abstract class Learn_ABPTestBase : AbpIntegratedTestBase<Learn_ABPTestModule>
    {
        ...
        protected void UsingDbContext(Action<Learn_ABPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Learn_ABPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Learn_ABPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Learn_ABPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Learn_ABPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Learn_ABPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Learn_ABPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Learn_ABPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Learn_ABPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Learn_ABPDbContext, Task<T>> func)
        {
            T result;
